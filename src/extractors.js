...
const yelpBusinessPartial = ($) => {
    ...
    const apolloState = html.match(/<script data-apollo-state.*>(.*)<\/script>/);
    console.log(apolloState[0]);
    console.log(apolloState[1]);
...
const yelpBusinessProps = (json) => {
    return {
        ...
        'businessHighlights': get(json, ['bizDetailsPageProps', 'sponsoredBusinessHighlightsProps', 'businessHighlights'], []).map((highlight) => highlight.title),
    }
};
